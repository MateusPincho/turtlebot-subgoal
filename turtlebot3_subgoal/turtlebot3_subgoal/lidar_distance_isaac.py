...
class LidarSectorNode(Node):
    ...
    def listener_callback(self, msg):
        ...
        def angle_to_index(angle_rad):
            ...
            return max(0, min(idx, len(ranges) - 1))
        ...
        for name, center_angle in self.sector_angles.items():
            
            # Special handling for SOUTH (Wraps around the array edges)
            if name == "SO":
                # Part 1: From start of array (-pi) to -pi + 22.5 deg
                idx_start_1 = 0 
                idx_end_1   = angle_to_index(msg.angle_min + half_sector)
                
                # Part 2: From pi - 22.5 deg to end of array
                idx_start_2 = angle_to_index(msg.angle_max - half_sector)
                idx_end_2   = len(ranges)
                
                sector_data[name] = ranges[idx_start_1:idx_end_1] + ranges[idx_start_2:idx_end_2]
            
            # All other sectors (NO, NW, NE, etc.) are continuous in [-pi, pi]
            else:
                ...
                sector_data[name] = ranges[start_idx:end_idx]
