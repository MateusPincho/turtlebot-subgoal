...
fuzzy_nav.add_term("Goal Orientation", "SOe", "Gaussian", [np.pi, 0.5])
fuzzy_nav.add_term("Goal Orientation", "SE", "Gaussian", [3.0 * np.pi / 4.0, 0.5])
fuzzy_nav.add_term("Goal Orientation", "ES", "Gaussian", [np.pi / 2.0, 0.5])
fuzzy_nav.add_term("Goal Orientation", "NE", "Gaussian", [np.pi / 4.0, 0.5])
fuzzy_nav.add_term("Goal Orientation", "NO", "Gaussian", [0.0, 0.5])
fuzzy_nav.add_term("Goal Orientation", "NW", "Gaussian", [-np.pi / 4.0, 0.5])
fuzzy_nav.add_term("Goal Orientation", "WE", "Gaussian", [-np.pi / 2.0, 0.5])
fuzzy_nav.add_term("Goal Orientation", "SW", "Gaussian", [-3.0 * np.pi / 4.0, 0.5])
fuzzy_nav.add_term("Goal Orientation", "SOw", "Gaussian", [-np.pi, 0.5])
...
fuzzy_nav.add_term("D*", "Near", "Gaussian", [0.3, 0.15])
...
fuzzy_nav.add_variable("Confidence", 0.0, 1.0)
fuzzy_nav.add_term("Confidence", "Low", "Gaussian", [0.0, 0.5])
fuzzy_nav.add_term("Confidence", "Medium", "Gaussian", [0.80, 0.1])
...
class FuzzyPlanner(Node):
    ...
    def distance_callback(self, msg: Float32MultiArray):
        ...
        if(self.old_wind_rose != None and abs(wind_rose["WE"]  - self.old_wind_rose["WE"] > 0.9)):
            wind_rose["WE"] = self.old_wind_rose["WE"]  # Append previous value if change is significant
        
        if(self.old_wind_rose != None and abs(wind_rose["NW"]  - self.old_wind_rose["NW"] > 0.9)):
            wind_rose["NW"] = self.old_wind_rose["NW"]  # Append previous value if change is significant

        if(self.old_wind_rose != None and abs(wind_rose["NO"]  - self.old_wind_rose["NO"] > 0.9)):
            wind_rose["NO"] = self.old_wind_rose["NO"]  # Append previous value if change is significant

        if(self.old_wind_rose != None and abs(wind_rose["NE"]  - self.old_wind_rose["NE"] > 0.9)):
            wind_rose["NE"] = self.old_wind_rose["NE"]  # Append previous value if change is significant

        if(self.old_wind_rose != None and abs(wind_rose["ES"]  - self.old_wind_rose["ES"] > 0.9)):
            wind_rose["ES"] = self.old_wind_rose["ES"]  # Append previous value if change is significant

        self.old_wind_rose = wind_rose
